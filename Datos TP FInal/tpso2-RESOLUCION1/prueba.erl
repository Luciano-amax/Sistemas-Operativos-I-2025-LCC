-module(prueba).
-export([inicioP2P/1, atentoUDP/3, atentoNameRequester/5,getIP/0,loop_receiveTCP/3, atentoTCP/4, comandos/6, descargarArch/4, procesoDeDescargas/3, helloPeriodicos/3, decodeMsg/1, atentoHola/2]).
...
atentoNameRequester(Id, Socket, Puerto, Ip,PidOrigen) ->
    ...
    after 
        5000 ->     io:fwrite("ENTRE~p~n",[PidOrigen]),
...
loop_receiveUDP(MapaNodos,MapaTempos,SocketUDP,PidNameRequester)->
    receive 
        {udp, SocketUDP, IP, Port, Message}-> 
            io:fwrite("Mensaje recibido: ~p desde ~p:~p~n", [Message, IP, Port]),
            case decodeMsg(Message) of
                {name_request, Id} ->
                    case maps:find(Id, MapaNodos) of 
                        {ok,_Valor} ->
                            Msg=string:concat("nvalid_name ",integer_to_list(Id)),
                ...
                {hello, Id, Puerto} ->
                    io:fwrite("recivi un hola de ~p~p~n",[Id,IP]),
                    case maps:find(Id, MapaNodos) of
                        {ok, _Valor} ->
...
loop_receiveTCP(SocketTCP,PidAtentoUDP,MyId)->
    {ok,NewSocketTCP}=gen_tcp:accept(SocketTCP),
    receive 
        {tcp,NewSocketTCP, Msg}->
            case decodeMsg(Msg) of
                {download_request, NombreArch}->
                    spawn(?MODULE, procesoDeDescargas, [NewSocketTCP, NombreArch]), % me llega un mensaje me fijo si es para descargar
...
atentoTCP(PidAtentoUDP,MyId,Puerto,PidOrigen)->
    {ok, SocketTCP} = gen_tcp:listen(Puerto, [binary, {active, true},{reuseaddr, true}]),
    PidOrigen!{pid,SocketTCP},
    loop_receiveTCP(SocketTCP,PidAtentoUDP,MyId).
...
accept_loop(SocketTCP, PidAtentoUDP, MyId) ->
    case gen_tcp:accept(SocketTCP) of
        {ok,Socket}->
    spawn(?MODULE, loop_receiveTCP, [Socket, PidAtentoUDP, MyId]),
    accept_loop(SocketTCP, PidAtentoUDP, MyId);
    _-> listo
    end.
