-module(udp_spawner).
-export([inicioP2P/1, atentoUDP/3, atentoNameRequester/5,getIP/0, atentoTCP/4, comandos/6, descargarArch/4, procesoDeDescargas/4, helloPeriodicos/3, decodeMsg/1, atentoHola/2]).
...
getIP() ->
    Cmd = "powershell.exe -Command \"(Get-NetIPAddress -AddressFamily IPv4 | Where-Object {$_.IPAddress -like '192.168.*'}).IPAddress\"",
    Output = os:cmd(Cmd),
    ...
    case Clean of
        "" ->
            {error, not_found};
        IPString ->
            Parts = string:split(IPString, ".", all),
            Numbers = [list_to_integer(string:trim(P)) || P <- Parts],
            {ok, list_to_tuple(Numbers)}
    end.
            ...
            case decodeMsg(Message) of
                {name_request, Id} ->
                    case maps:find(Id, MapaNodos) of 
                        {ok,_Valor} ->
                            Msg=string:concat("invalid_name ",integer_to_list(Id)),
                                            ...
                                            true -> % el archivo es menor a 4 Mg lo mando de una
                                                ...
                                                {ok, BinMsg}-> 
                                                        ...
                                                        % mensaje formado
                                                            ...
                                                            file:close(Raw);
                                                _-> 
                                                    io:fwrite ("se envio algooooooooooooooooooooooooooooooooooooooooooo~n"),
                                                    Error = "112",
                                                    MensajeError = list_to_binary(Error),
...
procesoDeDescargas(NuevoSocket, NombreArch, MiId, PidAtentoUDP)->
    ...
    case file:list_dir(Path) of
        {ok, ListaMisArchivos} -> case lists:member(NombreArch,ListaMisArchivos) of 
                                true -> PidAtentoUDP ! {tengoNodo,Id,self()},
                                        receive         %MANDARTCP
                                            {okEnviado, Ip, Port} -> io:fwrite("sfjsdjlkfsd ~p~n",[Ip]),
        ...
        {tcp, SocketTCP, Msg}->
            ...
            case decodeMsg(Msg) of
                {download_request, NombreArch}->
                    spawn(?MODULE, procesoDeDescargas, [SocketTCP, NombreArch, MyId, PidAtentoUDP]), % me llega un mensaje me fijo si es para descargar
...
accept_loop(ListenSocket, PidAtentoUDP, MyId) ->
    case gen_tcp:accept(ListenSocket) of
        {ok, Socket} ->
            ...
            loop_receiveTCP(Socket, PidAtentoUDP, MyId),
            accept_loop(ListenSocket, PidAtentoUDP, MyId);
        Error ->
